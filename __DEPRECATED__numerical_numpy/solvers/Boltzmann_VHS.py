...
def Boltzmann_VHS_kernel_modes_1D_integrand(
    ...
    _dim = 1
    if diagonal_only:
        _r = r.reshape(*utils.ones(_dim), -1)
    else:
        _r = r.reshape(*utils.ones(2*_dim), -1)
    _prod1 = \
        vhs_coeff * \
        np.power(2*utils.LAMBDA*v_max, _dim+vhs_alpha) * \
    ...
    r_freq1 = (utils.LAMBDA*np.pi) * np.linalg.norm(
                    freq_pair_1+freq_pair_2,
                    ord=2, axis=-1, keepdims=True
                )
    r_freq2 = (utils.LAMBDA*np.pi) * np.linalg.norm(
                    freq_pair_1-freq_pair_2,
                    ord=2, axis=-1, keepdims=True
                )
...
def Boltzmann_VHS_kernel_modes_3D_integrand(
    ...
    if diagonal_only:
        ...
        del(freq_pair_1, freq_pair_2, freq_pair)
        _prod2a = (4*np.pi) * utils.sinc(r_freq1 * _r)  # <-- Scale modification (Done: 3D)
        _prod2b = (4*np.pi) * utils.sinc(r_freq2 * _r)  # <-- Scale modification (Done: 3D)
        _prod2  = _prod2a * _prod2b
        ...
        r_freq1 = (utils.LAMBDA*np.pi) * np.sqrt(idx_pair[..., [0]])  # A 3-tensor
        r_freq2 = (utils.LAMBDA*np.pi) * np.sqrt(idx_pair[..., [1]])  # A 3-tensor
        del(idx_pair)
        _prod2a = (4*np.pi) * utils.sinc(r_freq1 * _r)    # <-- Scale modification (Done: 3D)
        _prod2b = (4*np.pi) * utils.sinc(r_freq2 * _r)    # <-- Scale modification (Done: 3D)
        _prod2  = _prod2a * _prod2b
...
def Boltzmann_VHS_kernel_modes(
    ...
    # Set the integrand
    func: Callable[[utils.ArrayData, dict[str, object]], utils.ArrayData]
    if dim==1:
