...
##################################################
##################################################
class FastSM_Boltzmann_VHS__Advanced(FastSM_Boltzmann__Advanced):
    r"""## The class for the fast spectral method for solving the homogeneous Boltzmann equation with the VHS model.
    ...
    def set_positivity_constant(self, density: torch.Tensor) -> None:
        ...
        lower_bound = \
            ...
            self._vhs_coeff * \
            ((2 * utils.LAMBDA * self._v_max)**(self._vhs_alpha))
        self._positivity_constant = lower_bound * aux_scale
        return
