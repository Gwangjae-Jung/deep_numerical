...
def compute_moments_inhomogeneous(
    ...
    ### Arguments
    * `f` (`numpy.ndarray`)
        The distribution function at a specific time, which is of shape `(B, N_1, ..., N_d, K_1, ..., K_d)`, *without an extra dimension at the end*.
    * `v` (`numpy.ndarray`)
        * The velocity grid.
        The velocity grid, which is of shape `(K_1, ..., K_d, d)`.
    * `eps` (`float`, default: `_EPSILON`)
    ...
    * `T` saves the mean temperature of each instance, which is of shape `(B, N_1, ..., N_d, 1)`.
    ...
    >>> max_x, max_v = 5, 10
    >>> num_x, num_v = 101, 201
    >>> grid_x, grid_v = np.linspace(-max_x, max_x, num_x), np.linspace(-max_v, max_v, num_v)
    >>> grid_xv = np.stack(np.meshgrid(grid_x, grid_v, indexing='ij'), axis=-1)
    >>> f = np.exp(-np.abs(grid_xv).sum(-1) / 4)
    >>> moments = compute_moments_inhomogeneous(f, grid_v[..., None])
    >>> f_equi = maxwellian_inhomogeneous(grid_xv, *moments)
    """
    # Retrieve the dimension and dv
    dim = v.shape[-1]
    dv  = float(np.prod(v[ones(dim)] - v[zeros(dim)]))
...
def compute_energy_homogeneous(
    ...
    ### Arguments
    * `f` (`numpy.ndarray`)
        The distribution function at a specific time, which is of shape `(B, K_1, ..., K_d)`, *without an extra dimension at the end*.
    * `v` (`numpy.ndarray`)
        The velocity grid, which is of shape `(K_1, ..., K_d, d)`.
    """
    # Retrieve the dimension and dv
