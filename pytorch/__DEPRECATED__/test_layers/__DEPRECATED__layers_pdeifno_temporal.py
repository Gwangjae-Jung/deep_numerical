...
class PDEiTemporalSpectralConv(nn.Module):
    """## PDE-inspired temporal spectral convolutional layer
    ...
    def pdei_spectral_matmul(self, fourier: torch.Tensor, _range: object) -> torch.Tensor:
        """
        ...
        (Note that the time dimension does not need to be considered, since only single-time data is passed.)
    ...
    def forward(self, X: torch.Tensor) -> torch.Tensor:
        ...
        for _range in product(*self.__list_range__):
            Y[_range] = self.pdei_spectral_matmul(X_rfftn, _range)
...
class PDEiTemporalFourierLayer(nn.Module):
    """## PDE-inspired temporal Fourier layer
    ### Kernel integration via FFT with a skip connection
    """
    ...
    def __init__(
        ...
        self.spectral = PDEiTemporalSpectralConv(n_modes = n_modes, hidden_channels = hidden_channels, n_modes_as = n_modes_as)
    ...
    def forward(self, Y_prev: torch.Tensor, Y_curr: torch.Tensor) -> torch.Tensor:
        """
        ### Remark
        Both `Y_prev` and `Y_curr` are of the shape `(B, C, s_1, ..., s_d)`.
        """
        spectral = self.spectral(Y_curr)
        return self.activation(Y_prev + spectral)
