...
class TempRSCL(nn.Module):
    """## Temporal reversible spectral convolutional layer (TempRSCL)
    ...
    def __init__(
        ...
        """## The initializer of the class `TempRSCL`
        -----
...
class TempRFL(nn.Module):
    """## Temporal reversible Fourier layer (TempRFL)
    ...
    def __init__(
        ...
        """## The initializer of the class `TempRFL`
        ...
        print("'TempRFL' will be deprecated.")
    ...
    def forward(self, X: torch.Tensor) -> torch.Tensor:
        return self.activation(self.spectral.forward(X))
