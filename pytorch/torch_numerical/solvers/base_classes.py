...
class SpectralMethodBase():
    ...
    def compute_fft(
        ...
        * `loss_fft` is the loss term in the Fourier space, computed using the method `compute_loss_fft`.
        """
        return (
            self.compute_gain_fft(_PLACEHOLDER__t_curr, fft_curr) - \
            self.compute_loss_fft(_PLACEHOLDER__t_curr, fft_curr)
        )
    ...
    def forward(
        ...
        homogeneous_result = RK_fcn(t_curr, f_fft, delta_t, self.compute_fft)
        if self._is_homogeneous:
...
##################################################
##################################################
class FastSM_Boltzmann_MultiRes(SpectralMethodBase):
    r"""## The base class for the fast spectral method for solving the Boltzmann equation.
    ...
    def __init__(
        ...
        * `dimension` (`int`)
            The dimension of the domain.
        * `v_num_grid` (`int`)
            The number of the grid points in each dimension. This class supports computation for inputs discretized with more thatn `v_num_grid` points.
        * `v_max` (`float`)
            The maximum velocity in the velocity space.
        * `x_num_grid` (`int`)
            The number of the grid points in each dimension.
        ...
        * `quad_order_lebedev` (`Optional[int]`)
        ...
        self.__kernel_slices:   tuple[tuple[slice]] = utils.freq_slices_low([v_num_grid for _ in range(self._dimension)])
        return
    ...
    def compute_gain_fft(
        ...
        _shape_of_summands = (*fft_curr.shape, *self._fsm_phase_1.shape[-2:])
        _dtype_and_device = {'dtype': fft_curr.dtype, 'device': fft_curr.device}
        x1 = torch.zeros(_shape_of_summands, **_dtype_and_device)
        x2 = torch.zeros(_shape_of_summands, **_dtype_and_device)
        for kernel_slice in product(*self.__kernel_slices):
            sub_fft_curr = fft_curr[..., *kernel_slice, :, None, None]
            sub_phase_1  = self._fsm_phase_1[..., *kernel_slice, :, :, :]
            sub_phase_2  = self._fsm_phase_2[..., *kernel_slice, :, :, :]
            sl_x = (..., *kernel_slice, *utils.repeat(slice(None), 3))
            x1[*sl_x] = sub_fft_curr * sub_phase_1
            x2[*sl_x] = sub_fft_curr * sub_phase_2
        conv = utils.convolve_freqs(x1, x2, self.v_axes)
        for kernel_slice in product(*self.__kernel_slices):
            _sl = (..., *kernel_slice, *utils.repeat(slice(None), 3))
            conv[*_sl] = self._fsm_scale[*_sl] * conv[*_sl]
        gain_fft = torch.sum(conv, dim=(-2, -1))
        return gain_fft
    ...
    def compute_loss_fft(
        ...
        x1 = torch.zeros(fft_curr.shape, dtype=fft_curr.dtype, device=fft_curr.device)
        x2 = torch.zeros(fft_curr.shape, dtype=fft_curr.dtype, device=fft_curr.device)
        for kernel_slice in product(*self.__kernel_slices):
            _sl = (..., *kernel_slice, slice(None))
            x1[*_sl] = fft_curr[*_sl]
            x2[*_sl] = self._kernel_diag[*_sl] * fft_curr[*_sl]
        loss_fft = utils.convolve_freqs(x1, x2, self.v_axes)
        return loss_fft


##################################################
##################################################
class FastSM_Landau_MultiRes(SpectralMethodBase):
    """## The class for the fast spectral method for solving the Fokker-Planck-Landau equation with the VHS model.
