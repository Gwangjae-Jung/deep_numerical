...
class MGKN(torch.nn.Module):
    ...
    def Upsample(self, x, channels, scale, s):
        ...
        x = F.interpolate(x, scale_factor=scale, mode='nearest') # (1, width, s) to (1, width,  s*2)
...
ntrain = 500 # number of training pairs N
...
TRAIN_PATH  = f"E:/PDE_datasets/from_pde_dataset/Burgers equation/Burgers_R10.npz"
TEST_PATH   = f"E:/PDE_datasets/from_pde_dataset/Burgers equation/Burgers_R10.npz"
...
for p in model.parameters():
    n += p.numel()
print(f"There are {n} parameters.")
...
for ep in tqdm(range(epochs)):
    ...
    for data in data_train:
        ...
        l2_loss = myloss(out.view(1, -1), y.view(1, -1))
        l2_loss.backward()
    ...
    scheduler.step()
    t2 = default_timer()
    print(f"Epoch {ep}: time {t2 - t1:.2f}, mse {train_mse / len(data_train):.3e}, rel {train_l2 / len(data_train):.3e}")
