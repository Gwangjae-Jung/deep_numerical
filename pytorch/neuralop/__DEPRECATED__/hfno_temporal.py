...
class TemporalHyperbolicFNO(nn.Module):
    ...
    def __init__(
        ...
        """## The initializer of the class `TemporalHyperbolicFNO`
        ----
        ### Arguments
    ...
    def forward(self, X_prev_curr: torch.Tensor) -> torch.Tensor:
        """Compute `X_next` using the second order Euler approximation.
    ...
    def reverse(self, X_curr_next: torch.Tensor) -> torch.Tensor:
        """Compute `X_prev` using the second order Euler approximation.
