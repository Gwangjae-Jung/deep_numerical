...
from    custom_modules.dtypes   import  PathLike
from    custom_modules.random   import  grf
from    custom_modules.utils    import  get_time_str




def generate_data__pendulum(
    ...
    def phi2(t: np.ndarray, u: np.ndarray, v: np.ndarray, f1: np.ndarray) -> np.ndarray:
        coeff_sin = -(f1 / ROD_LENGTH)
        return coeff_sin * np.sin(u)
    ...
    for instance_number in tqdm(range(1, NUM_INSTANCES + 1)):
        ...
        for idx, t_next in enumerate(t_grid[1:]):
            ...
            k1v = phi2(t_curr, u_curr, v_curr, f1_curr)
            # k2
            k2u = phi1(t_curr + STEP / 2, u_curr + STEP * k1u / 2, v_curr + STEP * k1v / 2)
            k2v = phi2(t_curr + STEP / 2, u_curr + STEP * k1u / 2, v_curr + STEP * k1v / 2, f1_mid)
            # k3
            k3u = phi1(t_curr + STEP / 2, u_curr + STEP * k2u / 2, v_curr + STEP * k2v / 2)
            k3v = phi2(t_curr + STEP / 2, u_curr + STEP * k2u / 2, v_curr + STEP * k2v / 2, f1_mid)
            # k4
            k4u = phi1(t_curr + STEP, u_curr + STEP * k3u, v_curr + STEP * k3v)
            k4v = phi2(t_curr + STEP, u_curr + STEP * k3u, v_curr + STEP * k3v, f1_next)
            # Update
