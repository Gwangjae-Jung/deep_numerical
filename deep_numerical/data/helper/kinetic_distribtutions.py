...
def sample_quantities(
        ...
        v_max:          float,
        t_center:       float = 1.0,
        v_perturb_rate: float = 0.2,
        t_perturb_rate: float = 0.2,
    ...
    assert isinstance(v_max, float) and v_max > 0.0
    assert isinstance(t_center, float) and t_center > 0.0

    v_perturb_max = v_max * v_perturb_rate
    t_perturb_max = t_center * t_perturb_rate
    ...
    t   = (
        t_center + \
        (t_perturb_max**0.5) * (2*torch.rand(batch_size, 1, **dtype_and_device) - 1)
    )**2
