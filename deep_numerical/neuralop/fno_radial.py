...
class ModifiedRFNO(BaseModule):
    ...
    def __init__(
        ...
        if n_layers <= 0:
            raise ValueError(f"'n_layers' is chosen {n_layers}, which is not positive.")
        if delta_t <= 0.0:
            raise ValueError(f"'delta_t' is chosen {delta_t}, which is not positive.")
    ...
    def forward(self, X: torch.Tensor) -> torch.Tensor:
        ...
        if self.__pos_enc:
            pos = positional_encoding(X.shape, 'radial', dtype=X.dtype, device=X.device)
            U = torch.cat((X, pos), dim=-1)
        U = self.network_lift.forward(U)
        U = self.network_hidden.forward(U)
        U = self.network_projection.forward(U)
        return X + self.__delta_t * U
