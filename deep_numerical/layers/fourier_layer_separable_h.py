...
class HyperSeparableSpectralConv(nn.Module):
    ...
    def __init__(
        ...
        #     ]
        # )
        self.hyper_kernel_components = nn.ModuleList(
            [
                MLP(
                    [n_parameters, 64, 64, prod(_shape)*rank]
                )
                for _shape in self.__kernel_shape
            ]
        )
    ...
    def __config_kernel(self) -> None:
        ...
        self.__kernel_einsum_cmd:   str = \
            ','.join(["b...ijr" for _ in self.__n_modes]) + "->b...ij"
    ...
    def forward(self, X: torch.Tensor, p: torch.Tensor) -> torch.Tensor:
        ...
        Arguments:
            ...
            `p` (`torch.Tensor`):
                * The tensor of the hyperparameters.
                * The shape of `p` is expected to be `(B, n_parameters)`.
        ...
        _hypernet: torch.nn.Module
        kernel_components = [
            torch.reshape(_hypernet.forward(p), 
                (
                    p.size(0),
                    *_shape,
            ...
            for _hypernet, _shape in zip(self.hyper_kernel_components, self.__kernel_shape)
        ]
        kernel = torch.einsum(self.__kernel_einsum_cmd, *kernel_components)
...
class HyperSeparableFourierLayer(nn.Module):
    ...
    def __init__(
        ...
        self.spectral   = HyperSeparableSpectralConv(n_modes, in_channels, out_channels, rank, n_parameters)
    ...
    def forward(self, X: torch.Tensor, p: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        Returns:
            `Y` (`torch.Tensor`): The transformed tensor after applying the MLP and the spectral convolution, with the residual connection.
            `p` (`torch.Tensor`): The tensor of the hyperparameters. This is returned so that `HyperSeparableFourierLayer` can be used with `torch.nn.Sequential`.
        """
        Y = X + self.mlp.forward( self.spectral.forward(X, p) )
        return Y, p
    
    
    def __repr__(self) -> str:
        return f"HyperSeparableFourierLayer(n_modes={self.n_modes}, in_channels={self.in_channels}, out_channels={self.out_channels}, rank={self.rank})"
