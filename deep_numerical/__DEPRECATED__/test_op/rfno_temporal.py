...
from    ..layers        import  LiftProject, SpectralConv
from    ..utils         import  *
...
class TempRFNO(nn.Module):
    """## Temporal reversible Fourier Neural Operator (TempRFNO)
    ...
    def __init__(
        ...
        """## The initializer of the class `TempRFNO`
        ----
        ### Arguments
    ...
    def forward(self, X_prev_curr: torch.Tensor) -> torch.Tensor:
        ...
        X_curr = self.network_spectral.forward(X_curr)
        
        # Project the updated current representation to the physical space
        return self.network_projection.forward(X_prev + X_curr)
    ...
    def reverse(self, X_curr_next: torch.Tensor) -> torch.Tensor:
        ...
        # Project the updated current representation to the physical space
        return self.network_projection.forward(X_next - X_curr)
    ...
    def __name__(self, full: bool = True) -> str:
        if full:
            ...
            return f"RFNO ({self.dim_domain}D, temporal)"
