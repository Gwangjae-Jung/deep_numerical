...
class FactorizedSpectralConv(nn.Module):
    ...
    def __init__(
        ...
        """The initializer of the class `FactorizedSpectralConv`

        Arguments:
            `n_modes` (`Sequence[int]`):
                The number of modes for each dimension.
            `in_channels` (`int`):
            ...
            `dtype` (`Optional[torch.dtype]`, default: `None`):
                The *real* data type of the model parameters.
        ...
        self.kernel_list = nn.ParameterList(
            [
                nn.Parameter(scale_linear*torch.randn(size=(n_mode, out_channels, in_channels), dtype=self.__dtype))
    ...
    def dtype_complex(self) -> torch.dtype:
        """Alias of `FactorizedSpectralConv.dtype`."""
        return self.__dtype
...
class FactorizedFourierLayer(nn.Module):
    ...
    def __init__(
        ...
        Arguments:
            ...
            `bias` (`bool`, default: `True`):
                Whether to use bias in the linear layers of the MLP.
            ...
            `dtype` (`Optional[torch.dtype]`, default: `None`):
                The data type of the model parameters. If `None`, then the default real data type is used.
        """
        super().__init__()
        if out_channels is None:
            out_channels    = in_channels
        ...
        self.spectral   = FactorizedSpectralConv(n_modes, in_channels, out_channels, dtype=dtype)
        self.mlp        = MLP(mlp_channels, bias=bias, activation_name=activation_name, activation_kwargs=activation_kwargs, dtype=dtype)
        return None
    ...
    def forward(self, X: torch.Tensor) -> torch.Tensor:
        ...
        Arguments:
            `X` (`torch.Tensor`): The input tensor of shape `(B, s_1, ..., s_d, C)`, where `B` is the batch size, `s_i` are the spatial dimensions, and `C` is the number of channels.
