...
class GalerkinTypeSelfAttention(nn.Module):
    ...
    def forward(self, X: torch.Tensor) -> torch.Tensor:
        ...
        X_kv = torch.einsum(f"b...hd,b...he->bhde", X_key, X_value)/num_points
        X_sa = torch.einsum(f"b...hd,bhde->b...eh", X_query, X_kv)
        X_sa = X_sa.reshape(X.shape)
    ...
    def __init__(
        ...
        Arguments:
            ...
            `mlp_hidden_channels` (`Sequence[int]`, default: `None`):
                ...
                * If `None`, then `mlp_hidden_channels` is set `[hidden_channels*2]`.
