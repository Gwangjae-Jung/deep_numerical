...
__all__ = ["SeparableSpectralConv", "SeparableFourierLayer"]
...
class SeparableSpectralConv(nn.Module):
    ...
    def __init__(
        ...
        self.kernel_components = nn.ParameterList(
            [
                torch.rand(size=(*_shape, out_channels, in_channels, rank), dtype=self.__dtype)
    ...
    def dtype_complex(self) -> torch.dtype:
        """Alias of `SeparableSpectralConv.dtype`."""
        return self.__dtype
...
class SeparableFourierLayer(nn.Module):
    ...
    def __init__(
        ...
        self.spectral   = SeparableSpectralConv(n_modes, in_channels, out_channels, rank, dtype)
        self.mlp        = MLP([out_channels, out_channels+in_channels, in_channels], bias=bias, activation_name=activation_name, activation_kwargs=activation_kwargs, dtype=dtype)
        return None
...
if __name__=="__main__":
    ...
    layer = SeparableFourierLayer([17, 17], 3, 5, activation_name='tanh', bias=False)
