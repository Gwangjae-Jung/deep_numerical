...
class TensorizedSpectralConv(nn.Module):
    ...
    def __init__(
        ...
        self.kernel_component_out   = nn.ParameterList(
            [
                torch.rand(size = (*_shape, out_channels, kernel_degree, kernel_rank,), dtype = torch.cfloat,)
                for _shape in self.__kernel_shape
            ]
        )
        self.kernel_component_in    = nn.ParameterList(
            [
                torch.rand(size = (*_shape, in_channels, kernel_degree, kernel_rank,), dtype = torch.cfloat,)
                for _shape in self.__kernel_shape
            ]
        )
