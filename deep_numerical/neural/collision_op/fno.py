from    typing      import  Literal, Sequence, Self, Optional
import  torch
    ...
    def __init__(
        ...
        Arguments:
            `n_modes` (`Sequence[int]`):
                ...
                To be precise, after performing the FFT, for the `i`-th Fourier transform, only the modes in `[-n_modes[i], +n_modes[i]]` will be preserved. Note that the length of `n_modes` is the dimension of the domain.
            `max_domain_value` (`float`):
                The maximum value of the domain in each dimension.
                Note that `max_domain_value` should be a positive real number.
            `hidden_channels` (`int`):
                The number of the hidden channels.
                It is required that `hidden_channels` is a positive *even* integer.
        ...
        for _ in range(n_layers):
            self.network_hidden.append(FourierLayer(**config_common, **config_fourier))
        ## Projection
