...
from    deep_numerical.fft  import  freq_pair_tensor, freq_index_pair_tensor
...
def Boltzmann_VHS_kernel_modes_3D_integrand(
    ...
    if diagonal_only:
        ...
        del(freq_pair_1, freq_pair_2, freq_pair)
        _prod2a = (4*torch.pi) * sinc(r_freq1 * _r)  # <-- Scale modification (Done: 3D)
        _prod2b = (4*torch.pi) * sinc(r_freq2 * _r)  # <-- Scale modification (Done: 3D)
        _prod2  = _prod2a * _prod2b
        ...
        r_freq1 = (LAMBDA*torch.pi) * torch.sqrt(idx_pair[..., [0]])  # A 3-tensor
        r_freq2 = (LAMBDA*torch.pi) * torch.sqrt(idx_pair[..., [1]])  # A 3-tensor
        del(idx_pair)
        _prod2a = (4*torch.pi) * sinc(r_freq1 * _r)    # <-- Scale modification (Done: 3D)
        _prod2b = (4*torch.pi) * sinc(r_freq2 * _r)    # <-- Scale modification (Done: 3D)
        _prod2  = _prod2a * _prod2b
